...
{
    internal class ArchiveCommand : IMaxCommand
    {
        public void RegisterCommand(Command maxCommand, JsonNode config)
        {
            ...
            var recursiveOption = new Option<bool>("--recursive", "[可选] 是否递归查找子目录");
            recursiveOption.SetDefaultValue(false);
            recursiveOption.AddAlias("-r");
            archiveCommand.Add(recursiveOption);

            // 添加操作
            archiveCommand.SetHandler((path, filter, yes, outDir, softLink, reverse, recursive) =>
            {
                // 检查 everything 是否运行
                ...
                // 获取 max 文件
                List<MaxFile> dotMaxFiles = GetDotMaxFiles(path, filter, outDir, recursive);
            ...
            }, pathsOption, grepOption, yesOption, outOption, softLinkOption, reverseOption, recursiveOption);
        }
        ...
        /// <returns></returns>
        private static List<MaxFile> GetDotMaxFiles(List<string> paths, string filter, string outDir, bool recursive)
        {
            // 为空时，使用当前环境目录
            if (paths == null)
            ...
            foreach (string path in paths)
            {
                ...
                if (Directory.Exists(path))
                {
                    ...
                    var maxFiles = Directory.GetFiles(path, "*.max", recursive ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly);
        ...
        private static void ArchiveMaxFile(MaxFile maxFile, bool yes, string outDir, bool softLink, bool reverseLink)
        {
            ...
            maxFile.FileAssets = GetAssetFiles(maxFileFullName);
        ...
        /// <summary>
        /// 参考：https://neiltech.wordpress.com/2013/04/07/reverse-engineering-the-max-file-format/
        /// </summary>
        /// <param name="cf"></param>
        /// <returns></returns>
        private static List<string> GetMapFileNamesFrom2010To2013(CompoundFile cf)
        {
            if (!GetAssetsBytes(cf, "FileAssetMetaData2", out var bytes))
            {
        ...
        /// <param name="cf"></param>
        /// <returns></returns>
        private static List<string> GetMapFileNamesSince2013(CompoundFile cf)
        {
            if (!GetAssetsBytes(cf, "FileAssetMetaData3", out var bytes))
            {
                return new List<string>();
            }
        ...
        /// <returns></returns>
        private static bool GetAssetsBytes(CompoundFile cf, string name, out byte[] bytes)
        {
            //cf.RootStorage.VisitEntries(item =>
            //{
            //    var name = item.Name;
